 ...
 * @author engin
 */
class bulk_mail_form extends component {

    public function render($parameter) {
        $this->set_component_name("bulk_mail_form");
    ...
    public function loadAction() {
        ...
        if (session::exists(CURRENT_USER_SESSION_NAME)) {
            if ($_REQUEST) {
                ...
                $nview = new view();
                component::import_component("bulk_mail_form", ["type" => "extension/Bulkmail"], true);
                $nview->add_page(component::get_component_module());
                $nview->prepare_page();
                $content = $nview->get_content();
                $data["content"] = $content["bulk_mail_form"];
