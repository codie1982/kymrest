 ...
 * @author engin
 */
class admin_upgrade_pack extends controller {
    ...
    public function indexAction($parameters = array()) {
        component::import_component("pack", ["type" => "extension/update_admin"]);
        $this->view->add_page(component::get_component_module());

        $this->view->prepare_page();
        $content = $this->view->get_content();
//        $row = html::addrow([["12,12,12,12" => ["html" => $content["pack"]]]]);


        tema::column("12,12,12,12", $content["pack"]);
        tema::addrow();
