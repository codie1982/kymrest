 ...
 * @author engin
 */
class transport_adres_block extends component {
    public function render($parameter) {
        $this->add_global_plugin("makeForm");
        $this->set_component_name("transport_adres_block");
    ...
    public function removeAction() {
        ...
        if (session::exists(CURRENT_USER_SESSION_NAME)) {
            if ($_POST) {
                $key = $_POST["key"];
                $nsettings_transport_location = new table_settings_transport_location();
                $nsettings_transport_location->remove($key);
                $data["sonuc"] = true;
