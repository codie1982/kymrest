...
<div data-section="tag">
    <div class="form-group">
        ...
        <div class="tag_content">
            ...
            if (!empty($customer_tag_data)) {
                foreach ($customer_tag_data as $data) {
                    ...
                    for ($i = 0; $i < count($customer_tag_list); $i++)
                        $taglist[] = ["text" => $customer_tag_list[$i], "attributes" => ["value" => $customer_tag_list[$i], "!" => $data->tag == $customer_tag_list[$i] ? "selected" : "Kullanıcı Etiketleri Oluşturun"]];
                    $col[] = [
                        "col" => ["2"],
                        "content" => [
                            "type" => "select",
                    ...
                    echo '<input type="hidden" data-block_tag_personel data-json="' . base64_encode(json_encode($template)) . '"/>';
                }
            }
            ...
            echo '<input type="hidden" reset-block_tag_personel data-json="' . base64_encode(json_encode($resettemplate)) . '"/>';
            ?>
        </div>
    </div>
</div>
