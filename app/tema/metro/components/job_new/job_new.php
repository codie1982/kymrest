 ...
 * @author engin
 */
class job_new extends component {

    public function render($parameter) {
        $this->set_component_name("job_new");
    ...
    public function loadAction() {
        ...
        if (session::exists(CURRENT_USER_SESSION_NAME)) {
            if ($_REQUEST) {
                ...
                component::add_props(["customer_id" => $customer_id, "customer_name" => $customer_name]);

                $nview = new view();

                component::import_component("job_new", ["type" => "admin"], true);
                $nview->add_page(component::get_component_module());
                $nview->prepare_page();
                $content = $nview->get_content();


                $data["content"] = $content["job_new"];
