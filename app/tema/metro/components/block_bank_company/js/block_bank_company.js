var block_bank_company = function () {
    var customer_bank = function () {
        let options = {
        ...
        if (options.content !== "")
            $(".bank_content").makeForm(options);
        $('[data-bank="add"]').on("click", function (e) {
            e.preventDefault();
            let options = {
                component_name: "block_bank_company",
                secret_number: true,
            ...
            var predata = document.querySelector('[reset-' + options.component_name + ']');
            options.content = (JSON.parse(atob(predata.getAttribute("data-json"))));
            $(".bank_content").makeForm(options).addnew();
        })
    }
    return {
        init: function () {
...
jQuery(document).ready(function () {
    block_bank_company.init(); // init metronic core componets
});