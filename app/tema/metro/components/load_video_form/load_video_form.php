 ...
 * @author engin
 */
class load_video_form extends component {

    public function render($parameter) {
        $this->set_component_name("load_video_form");
    ...
    public function loadAction() {
        ...
        if (session::exists(CURRENT_USER_SESSION_NAME)) {
            if ($_REQUEST) {
                ...
                $nview = new view();
                component::import_component("load_video_form", ["type" => "admin"], true);
                $nview->add_page(component::get_component_module());
                $nview->prepare_page();
                $content = $nview->get_content();
                $data["content"] = $content["load_video_form"];
